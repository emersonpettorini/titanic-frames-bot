...
- **GitHub Actions** (`.github/workflows/post.yml`): cron de 30 min chama o
  `post.py` e commita o estado de volta. Os frames ficam num repo público
  separado, servidos por `raw.githubusercontent.com`.
...
- O cron `*/30 * * * *` posta e commita o `state.json` avançado.
